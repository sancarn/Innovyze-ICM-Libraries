...
def subnetworks(network)
    initial = network.row_objects("hw_pump").map do |pump_link|
        {
            "sps" => pump_link,
            "us_network" => pump_link.navigate("all_us_links"),
            "subnetwork" => pump_link.navigate("all_us_links")
        }
    end

    smallest_to_largest = initial.sort_by {|sps| sps["us_network"].length}
    
    #Step through networks from smallest to largest, subtracting all small networks from the larger ones to find the subnetworks
    smallest_to_largest.each_with_index do |sps, index|
        ids = sps["us_network"].map {|link| link.id }
        smallest_to_largest[index+1..-1].each do |subnet|
            subnet["subnetwork"] = subnet["subnetwork"].reject do |link|
                ids.include?(link.id)
            end
        end
    end
...
def getSPSFloodLevelData(network)
    allSPS = subnetworks(network).map do |subnet|
        pump_link = subnet["sps"]
        feeding_network = subnet["subnetwork"]
        ...
        csos = feeding_network.select do |link|
            #Ensure ds pipes > 1, then check number of outfalls > sps outfalls
            #note: if the first condition is not met, the second condition will not be checked
            link.ds_node.ds_links.length > 1
        end.map do |link|
            link.ds_node
        end.select do |cso_candidate|
            outfall_sets =  cso_candidate.navigate("ds_links").map {|link| getOutfalls(link).map {|outfall| outfall.id }.sort}
            outfall_sets.combination(2).any? { |a, b| a != b }
        end
        ...
        spills = csos.map do |cso_node|
            ...
            #Bifurcation check - flood_level can still be nil in the following case:
            #|
            #|\
            #|  |
            #|  |
            ...
            #|  |
            #|  |
            #|/   < currently captured as cso, despite cso being a bit more downstream of this link
            #|
            #in this case there is no cso. So reject it.
            cso["flood_level"].nil?
        end
